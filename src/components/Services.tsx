...
export default function Services() {
  ...
  const handleCategoryChange = (catId: string, label: string) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16" id="services-header">
          ...
          <p className="text-slate-600 dark:text-slate-400 font-light text-base sm:text-lg">
            We architect, deploy, and scale digital assets for organizations requiring peak throughput, rigid database integrity, and premium layouts.
          </p>
        </div>

        {/* Categories Tab Selector */}
        <div className="flex flex-wrap justify-center items-center gap-2 mb-12" id="services-category-tabs">
          {categories.map((cat) => (
            <button
              key={cat.id}
        ...
        <div className="mt-16 p-8 rounded-2xl bg-gradient-to-r from-blue-500/5 via-violet-500/5 to-pink-500/5 border border-blue-500/10 flex flex-col md:flex-row items-center justify-between gap-6" id="services-global-reach">
          <div className="space-y-1 text-center md:text-left">
            ...
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-light">
              We offer personalized systems engineering consultation to design precise databases, compliance logic, and scalable deployments.
            </p>
          </div>
          <button
            onClick={() => {
