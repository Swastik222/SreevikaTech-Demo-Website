...
export default function CaseStudies() {
  ...
  const [sharingStudyId, setSharingStudyId] = useState(null);
  ...
  useEffect(() => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      trackEvent("search", `Search Case Studies: "${searchQuery}"`, {
        query: searchQuery,
      });
    }, 800);
  ...
  const handleShareClick = (e, studyId) => {
    e.stopPropagation();
    setSharingStudyId(sharingStudyId === studyId ? null : studyId);
    trackEvent("click", `Toggle Share Menu: CS-${studyId}`);
  };

  const handleSocialShare = (platform, studyTitle) => {
    setSharingStudyId(null);
    trackEvent("share", `Shared Case Study [${studyTitle}] via ${platform}`);
    alert(
      `Thank you for sharing! Content link formatted for ${platform} has been logged.`,
    );
  };

  const handleCopyLink = (e, studyId, title) => {
    e.stopPropagation();
    ...
    navigator.clipboard
      ...
      .then(() => {
        ...
        trackEvent(
          "share",
          `Copied link to clipboard for Case Study: ${title}`,
        );
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="max-w-xl space-y-4 text-center md:text-left">
            ...
            <p className="text-slate-600 dark:text-slate-400 font-light text-sm sm:text-base">
              Explore our engineering case studies to see how we help clients
              optimize complex processes, secure data pipelines, and achieve
        ...
        {filteredStudies.length === 0 ? (
          <div
            className="text-center py-16 bg-transparent rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm"
            id="empty-search-state"
          >
            <p className="text-slate-500 dark:text-slate-400 text-base">
              No case studies match your query:{" "}
            ...
            {filteredStudies.map((study, index) => (
              ...
              >
                ...
                <div className="p-6 sm:p-8 space-y-6">
                  ...
                  <div
                    className="flex items-center justify-between text-xs font-mono font-bold text-slate-500 dark:text-slate-400"
                    id="case-study-tags"
                  >
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-500/5 to-purple-500/5 rounded-xl border border-blue-500/10 flex items-center justify-between gap-4">
                    <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">
                      Recorded Output Metric:
                    </span>
                    <div className="flex items-baseline gap-1">
                      <span className="text-2xl font-sans font-extrabold text-blue-600 dark:text-blue-400">
                        {study.impactValue}
                      </span>
                      <span className="text-[10px] text-slate-500 font-mono font-bold uppercase">
                        {study.impactMetric.split(" ")[0]}
                      </span>
                    </div>
                  </div>
                ...
                {/* Card Footer Actions */}
                <div
                  className="px-6 py-4 bg-slate-50/50 dark:bg-slate-900/40 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between"
                  id="case-study-card-footer"
                >
                  {/* Read More Button */}
                    ...
                    <AnimatePresence>
                      {sharingStudyId === study.id && (
                        <motion.div
                          ...
                          className="absolute right-0 bottom-full mb-2 z-20 bg-transparent border border-slate-100 dark:border-slate-800 rounded-xl p-2 shadow-xl flex items-center gap-1"
                          onClick={(e) => e.stopPropagation()}
                          id={`share-popover-${study.id}`}
                        >
                          <button
                            onClick={() =>
                              handleSocialShare("LinkedIn", study.title)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleSocialShare("X (Twitter)", study.title)
                            }
                          ...
                          <button
                            onClick={() =>
                              handleSocialShare("Facebook", study.title)
                            }
                          ...
                          <button
                            onClick={(e) =>
                              handleCopyLink(e, study.id, study.title)
                            }
          ...
          {selectedStudy && (
            ...
            >
              <motion.div
                ...
                className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl border border-white/50 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] w-full max-w-3xl rounded-3xl overflow-hidden relative max-h-[90vh] flex flex-col"
                onClick={(e) => e.stopPropagation()}
              >
                {/* Modal Header */}
                <div className="p-6 md:p-8 bg-transparent border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-xs text-slate-500 font-medium">
                      <span>
                        Client: <strong>{selectedStudy.client}</strong>
                      </span>
                      <span>•</span>
                ...
                <div
                  className="p-6 md:p-8 overflow-y-auto space-y-6 text-sm"
                  ...
                  <div className="grid sm:grid-cols-3 gap-4 p-5 bg-gradient-to-br from-blue-500/5 to-purple-500/5 rounded-2xl border border-blue-500/10">
                    <div className="sm:col-span-2">
                      <span className="font-semibold text-slate-900 dark:text-white block text-sm">
                        Peak Outcomes Recorded
                      </span>
                      <span className="text-slate-500 dark:text-slate-400 text-xs font-light block">
                        Sreevika Tech solutions successfully validated.
                      </span>
                    </div>
                    <div className="text-left sm:text-right flex flex-col justify-center">
                      <span className="text-3xl font-sans font-black text-blue-600 dark:text-blue-400">
                        {selectedStudy.impactValue}
                      </span>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <ul className="grid sm:grid-cols-1 gap-3">
                      {selectedStudy.results.map((res, index) => (
                        <li
                          key={index}
                          className="flex items-start gap-2.5 p-3 rounded-xl bg-transparent border border-slate-100 dark:border-slate-800"
                        >
                ...
                {/* Modal Footer CTA */}
                <div className="p-6 bg-transparent border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
