...
import { ArrowRight, Terminal, Globe, Cpu, Sparkles, CodeXml } from 'lucide-react';
import { trackEvent } from '../utils/analytics';

export default function Hero() {
  const handleCtaClick = (sectionId: string, label: string) => {
  ...
  return (
    <section 
      className="relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden bg-slate-50 dark:bg-slate-900 transition-colors duration-200"
      id="home"
    >
          ...
          <div className="lg:col-span-7 space-y-8 text-center lg:text-left" id="hero-content">
            ...
            <div className="space-y-4">
              <motion.h1 
                ...
                id="hero-heading"
              >
                Engineering High-Performance <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent dark:from-blue-400 dark:via-purple-400 dark:to-pink-400">Software Solutions</span> for the Global Frontier.
              </motion.h1>
              ...
              <motion.p 
                ...
                id="hero-description"
              >
                Sreevika Tech is a premium software engineering firm delivering mission-critical web applications, scalable cloud microservices, and bespoke intelligence nodes to clients globally. Built with speed, safety, and absolute reliability.
              </motion.p>
            </div>
            ...
            >
              <button
                onClick={() => handleCtaClick('case-studies', 'Explore Case Studies')}
              ...
              <button
                onClick={() => handleCtaClick('contact', 'Request Consultation')}
            ...
            >
              {[
                { label: 'Uptime SLA', val: '99.99%', sub: 'Enterprise guarantee' },
                { label: 'Global Clients', val: '24/7', sub: 'Seamless coverage' },
                { label: 'Production Apps', val: '40+', sub: 'Deployed in Cloud' },
                { label: 'Core Engineers', val: 'Elite', sub: 'Advanced stack' },
              ].map((hl, i) => (
                <div key={i} className="text-center lg:text-left p-2.5 rounded-xl hover:bg-white dark:hover:bg-slate-900/40 transition-colors duration-200">
                  <span className="block text-2xl font-sans font-extrabold text-slate-900 dark:text-white">{hl.val}</span>
                  <span className="block text-xs font-semibold text-blue-600 dark:text-blue-400 mt-0.5">{hl.label}</span>
                  <span className="block text-[10px] text-slate-500 dark:text-slate-500 mt-0.5">{hl.sub}</span>
                </div>
              ))}
            </motion.div>
            ...
            >
              ...
              <div className="p-6 space-y-4 max-h-[380px] overflow-y-auto no-scrollbar text-[11px] sm:text-xs">
                <div className="flex items-start gap-2">
                  ...
                  <span className="text-blue-300">npx init sreevika-digital-core</span>
                </div>
                <div className="text-emerald-400 flex items-center gap-1.5 font-semibold">
                  ...
                  <span>[SUCCESS] Connection authorized on secure tunnel...</span>
                </div>
                ...
                <div className="bg-slate-900/80 p-3.5 rounded-xl border border-slate-800/60 font-mono text-slate-400 space-y-1 text-[11px]">
                  <div><span className="text-blue-400">"performance"</span>: <span className="text-pink-400">"sub-second loads"</span>,</div>
                  <div><span className="text-blue-400">"geographies"</span>: [<span className="text-emerald-400">"USA"</span>, <span className="text-emerald-400">"EU"</span>, <span className="text-emerald-400">"APAC"</span>],</div>
                  <div><span className="text-blue-400">"methodologies"</span>: <span className="text-pink-400">"Agile / Test-Driven"</span>,</div>
                  <div><span className="text-blue-400">"standards"</span>: [<span className="text-emerald-400">"HIPAA"</span>, <span className="text-emerald-400">"SOC2"</span>, <span className="text-emerald-400">"PCI-DSS"</span>]</div>
                </div>
