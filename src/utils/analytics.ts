import { Inquiry, NewsletterSubscriber, AnalyticsLog, AnalyticsSummary } from '../types';
...
const createSeedLogs = (): AnalyticsLog[] => {
  const seed: AnalyticsLog[] = [];
  const now = new Date();
  
  // Seed page views and interactions for the last 7 days
  const services = ['Custom Software', 'Cloud Architecture', 'Mobile Apps', 'AI Integrations', 'DevOps Automations'];
  const caseStudies = ['Global Logistics Platform', 'Fintech Security Engine', 'Healthcare IoT Core', 'AI Demand Planner'];
  ...
  for (let i = 7; i >= 0; i--) {
    ...
    for (let v = 0; v < viewsCount; v++) {
      ...
      seed.push({
        id: `seed-pv-${i}-${v}`,
        type: 'pageview',
        label: v % 5 === 0 ? 'Home Section' : v % 5 === 1 ? 'Services Section' : v % 5 === 2 ? 'Case Studies Section' : 'Contact Section',
    ...
    for (let c = 0; c < clickCount; c++) {
      const clickTime = new Date(date.getTime() + Math.random() * 24 * 60 * 60 * 1000);
    ...
    // Search queries
    if (Math.random() > 0.4) {
      const qTime = new Date(date.getTime() + Math.random() * 24 * 60 * 60 * 1000);
    ...
    for (let cs = 0; cs < csCount; cs++) {
      const csTime = new Date(date.getTime() + Math.random() * 24 * 60 * 60 * 1000);
    ...
    // Social share clicks
    if (Math.random() > 0.7) {
      const shareTime = new Date(date.getTime() + Math.random() * 24 * 60 * 60 * 1000);
      seed.push({
        id: `seed-share-${i}`,
        type: 'share',
...
const createSeedInquiries = (): Inquiry[] => {
  return [
    {
      id: 'seed-inq-1',
      name: 'Sarah Jenkins',
      ...
      message: 'Looking to migrate our legacy microservices stack to GCP with Kubernetes support. We are looking for a reliable partner in India with global reach.',
    ...
    },
    {
      ...
      message: 'Interested in building an end-to-end fintech dashboard with high security protocols. Needs a responsive and pixel-perfect design.',
      timestamp: new Date(Date.now() - 1.5 * 24 * 60 * 60 * 1000).toISOString(),
    }
  ];
};

export const getLogs = (): AnalyticsLog[] => {
...
export const saveInquiry = (inquiryData: Omit<Inquiry, 'id' | 'timestamp'>): Inquiry => {
  ...
  // Track this event
  trackEvent('contact_submit', `Inquiry by ${newInquiry.name} (${newInquiry.service})`);
  
  return newInquiry;
};

export const saveSubscriber = (email: string): boolean => {
...
export const getAnalyticsSummary = (): AnalyticsSummary => {
  ...
  const popularServices: { [service: string]: number } = {};
  const popularCaseStudies: { [caseStudy: string]: number } = {};
  ...
  return {
    ...
    popularServices,
    popularCaseStudies,
    recentInquiries: [...inquiries].sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).slice(0, 5),
  };
};
